...
- Recommendation: Implement the approved domain-claim operating model in the workflow design: first verified claimant of an unclaimed client business email domain can create the company and become initial Client Admin; Gmail/public-email signups can request company creation but require Admin review with alternate proof of company and administrative identity; later same-domain users enter Client Admin approval; Admins get an override queue for stale, invalid, or unavailable previous Client Admins. Product Ops still needs to define safe self-edit fields, proof requirements, blocked non-public domains, related-domain alias approval, and the admin repair audit trail.
- Acceptance criteria: Product has a documented domain-claim, public-email proof, and same-domain approval flow; Admins have a visible override/repair workflow; safe self-edit fields are named; proof requirements, blocked-domain, and related-domain decisions are documented; and QA has allow/deny cases for profile bootstrap.
...
- Product has confirmed the core profile bootstrap rule: first verified user for an unclaimed client business email domain may create the company and become initial Client Admin; Gmail/public-email users may request company creation but require Admin review and alternate proof; later same-domain users require Client Admin approval; Admin can override stale or invalid prior admins. Remaining Product Ops decisions: safe self-edit fields, exact public-email proof requirements, blocked non-public domains, related-domain aliases, role-assignment audit trail, and repair queue design.
...
- Need Product and Security confirmation for self-editable profile fields, exact public-email proof requirements, blocked non-public domains, related-domain aliases, saved-target visibility, extension-capture retention, converted capture visibility, and intro-request closure authority.
