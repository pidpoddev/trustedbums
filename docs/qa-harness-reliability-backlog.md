...
The role split is now proven and should stop being tracked as unfinished implementation work. GitHub `E2E Smoke` run `27110757594` on commit `8fa0796` completed green on 2026-06-08 with smoke plus `Deep QA (admin|client|bum)`, so the earlier auth/bootstrap regression is no longer an active harness item. Local implementation now makes `qa:target-preflight` write `summary.json` and `summary.txt` under `test-results/qa-target-preflight/` before it exits, so the remaining harness work is narrower: verify that artifact path in the next hosted preflight failure, make the GitHub deep-shard env contract match the standalone deep-audit contract, and stop silently skipping extension coverage in hosted preflight when the workflow leaves `QA_EXTENSION_API_BASE_URL` unset.
