...
- For GitHub-hosted `QA`, `E2E Smoke`, `Visual UI Audit`, and `Deep QA Hotfix Audit`, keep `QA_BASE_URL` on `https://trustedbums.com` unless Ryan explicitly asks to validate another deployed host. Keep runner-side `rcdl.tplinkdns.com` DNS/TLS/app-shell checks separate from hosted `https://trustedbums.com` release evidence.
