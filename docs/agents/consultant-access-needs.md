...
- Current status: This remains session-variable on 2026-06-15, so keep the request open. The current Security Engineer shell confirmed project `vaoqvtxqvbptyxddpoju` is `ACTIVE_HEALTHY`, returned the live project URL, exposed current security advisors, edge-function inventory, deployed `sync-claim-decision-replies` source, current edge-function logs, direct SQL for helper grants and tracker rows, and refreshed live tracker rows `TB-0023`, `TB-0081`, `TB-0085`, `TB-0087`, and `TB-0091` to exact head `7ee97c1`. The same-day shell also rechecked the current GitHub `QA`, deploy, `E2E Smoke`, and `Visual UI Audit` runs for that head. Earlier specialist history still shows variability, including shells that could read only metadata or that hit connector `RATE_LIMITED` errors while refreshing tracker rows. The remaining problem is therefore stability rather than base project reachability.
...
- Current status: These sources are still not present in the repo or connected tools as of 2026-06-15. Exact head `7ee97c1` still conflicts on `Clients We Represent` vs `Prospects` vs `Client Prospect`, `Bum Prospect` vs `Apply as a Bum`, `Client Agreement` vs `Partner Terms`, `Commission Plans` vs `commission structure` vs `commission schedule`, and `Deal Registration Process` vs `Deal Registration Automation` vs `Deal Registration Beta`, so content recommendations remain implementation-ready but product-code-backed rather than business-approved.
...
- Current status: GA4 setup is now API-verified under `TB-0066`: web stream `Trusted Bums Web`, stream URL `https://trustedbums.com`, stream ID `15031871944`, measurement ID `G-P6B5EYQMVN`, and property id `540873763`. Current source keeps GA consent-gated, sends manual SPA page views for public and authenticated routes, strips portal route IDs and sensitive query strings before sending `page_path`, and emits a `trustedbums_route_view` event with aggregate-safe parameters: `portal_area`, `route_group`, `auth_gate`, and `is_portal_route`. On 2026-06-15, Codex selected the Chrome profile named `Trusted Bums` for the bums@trustedbums.com GA check, but live GA UI access could not be verified because `https://analytics.google.com` was blocked by browser enterprise policy before the UI loaded. The repo now includes a browserless GA API path in `docs/google-analytics-api.md` and `scripts/google-analytics-api.mjs`; Google Cloud project `project-d45d35fc-184a-43c2-889` has `analyticsdata.googleapis.com`, `analyticsadmin.googleapis.com`, `iam.googleapis.com`, and `iamcredentials.googleapis.com` enabled; service account `trusted-bums-ga4-agent@project-d45d35fc-184a-43c2-889.iam.gserviceaccount.com` is configured for keyless impersonation by `bums@trustedbums.com`; and GA property access now works. API evidence from 2026-06-15 showed `25` active users, `25` sessions, `58` screen page views, and `282` events for 2026-06-15 so far, plus route rows including `/client/dashboard`, `/admin`, and `/bum/dashboard`. The four custom dimensions for `portal_area`, `route_group`, `auth_gate`, and `is_portal_route` already exist. Remaining watch item: the `trustedbums_route_view` event and its portal custom-dimension report returned zero rows in the 7-day API check, so agents can use GA for aggregate page-path and event reporting now but should not rely on the custom portal-event report until post-deploy consented portal traffic populates it.
