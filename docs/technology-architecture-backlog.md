...
- Recommendation: Run mobile-readiness as a planning and hardening sprint before implementation. Decide between responsive web/PWA-first, Capacitor wrapper, or Expo/React Native only after product scope, mobile auth/session handling, mobile API lane coverage, push/deep-link/offline needs, app-store privacy requirements, and QA coverage are documented.
...
- Migration or rollback notes: Keep the current responsive portal as the source of truth until a mobile path is accepted. If a Capacitor or Expo spike fails auth/deep-link/QA requirements, stay PWA-first and continue improving mobile web workflows.
- Acceptance criteria: `TB-0099` has a written mobile product scope, selected delivery path, auth/deep-link/session plan, API lane map, app-store/privacy checklist, and mobile QA smoke plan; existing mobile blockers are closed or explicitly waived.
