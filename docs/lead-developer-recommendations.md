...
- Dependencies/risks: Product has now defined first-time workspace creation by verified company email domain: first claimant of an unclaimed client domain may create the company and become initial Client Admin; later same-domain users require Client Admin approval or Admin override. Remaining decisions are exact self-editable fields, blocked/public domain list, related-domain alias policy, and admin repair tooling. Roll out with a rollback plan for legitimate onboarding failures.
...
- Security, Data, QA, and Product Ops should use the updated Profile Bootstrap business rule before profile/RLS hardening ships: domain-claim workspace creation, Client Admin approval for later same-domain users, and Admin override for stale or invalid prior admins.
