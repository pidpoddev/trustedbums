...
- Dependencies/risks: Product has now defined the first implementation rules: first claimant of an unclaimed client business email domain may create the company and become initial Client Admin; Gmail/public-email company creation requires Admin review and alternate proof; later same-domain users require Client Admin approval or Admin override; Client Admins may manage same-company users, including disabling another Client Admin; related domains may be requested but require validation before access; safe self-service fields are display name, timezone, date format, and notification preferences. Roll out with a rollback plan for legitimate onboarding failures.
...
- Security, Data, QA, and Product Ops should use the updated Profile Bootstrap business rule before profile/RLS hardening ships: domain-claim workspace creation, public-email manual verification, Client Admin same-company user management, related-domain validation, pending/denied states, audit events, and Admin override for stale or invalid prior admins.
