...
- Older `.env.qa` absence claims are stale because the file is now restored locally. Hosted authenticated role smoke and portal interaction audit now pass against `trustedbums.com`, closing the stale E2E smoke failures recorded from 2026-06-04. Sourced `qa:env` now correctly fails until `QA_EXTENSION_API_TOKEN` is added for the configured extension API base URL.
