...
- Dependencies/risks: Product must confirm first-time workspace creation and exact self-editable fields. Roll out with a rollback plan for legitimate onboarding failures and admin repair tooling.
...
- Security, Data, QA, and Product Ops all depend on the Profile Bootstrap business rule before profile/RLS hardening can safely ship.
