...
The active security queue is now down to `TB-0023`. `TB-0089` is no longer a repo-source or sampled live-deployment gap: live `extension-api-v1` v7, `portal-contacts` v5, `profile-bootstrap` v5, `admin-access-requests` v5, and `bum-extension-download` v3 now show the allowed Clerk issuer path, and a local source scan found no remaining token-selected issuer helper under `supabase/functions`.
...
- `Authorization-bearing admin and profile workflows`: the repo side and sampled live Edge Function source now agree on the allowed-issuer Clerk verification path for the `TB-0089` function set. Keep closing proof tied to live function reads, not static deploy evidence alone.
...
- Current `TB-0089` closeout conclusions are stronger than the prior refresh because they are backed by both repo-source review and sampled live deployed source, not just repo grep plus inventory.
...
- Results: hosted exact-head QA, deploy, and E2E are green on `a17a856`; dependency audit returned zero production advisories in the prior security pass; this closeout batch passed the full unit suite plus build; live SQL keeps `TB-0108` and `TB-0111` closed; `TB-0023` stays blocked pending Auth-setting visibility or explicit risk acceptance; and `TB-0089` is ready to close with live privileged-function issuer proof after the final commit is pushed.
