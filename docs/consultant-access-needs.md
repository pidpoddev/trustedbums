...
- Current status: This is no longer supported as a blanket local-routability failure. In the current QA run on 2026-06-08, sourced `corepack pnpm run qa:target-preflight` passed DNS and returned HTTP `200` plus the Trusted Bums app shell for `https://trustedbums.com`. GitHub `E2E Smoke` run `27109958355` then passed smoke plus all three Deep QA shards on 2026-06-08, while the next run `27110095517` failed only `Deep QA (client)` during hosted preflight with `fetch failed` and missing app shell. The remaining need is dashboard-grade evidence and root-cause tooling for intermittent hosted target-health mismatches, not proof that the site is generally unreachable.
...
- Current status: This narrowed again on 2026-06-07, but it is still only partial. Supabase MCP/connector access confirmed project URL `https://vaoqvtxqvbptyxddpoju.supabase.co`, live function inventory/source for `send-website-email`, `submit-contact`, and `email-track`, and follow-up deployed `email-track` version `2` matching the hardened local allowlist implementation. That was enough to close the earlier `email-track` deploy-drift finding, but not enough to refresh grants, RLS, helper-function reachability, or safe aggregates because `execute_sql` was not callable in the same session. Keep this request open until consultant sessions can reliably use read-only SQL plus current function inventory/source in the same run.
...
- Current status: This gap is narrower than the older failure trail. Hosted authenticated smoke now reaches and passes the role flows in the latest completed green `E2E Smoke` run `27109958355` on 2026-06-08, and current local target preflight reaches `https://trustedbums.com` after sourcing `.env.qa`. Clerk logs are still useful for diagnosing future intermittent sign-in failures, but they are no longer blocking the base authenticated smoke path in the current evidence set.
...
- Current status: This is now a narrow, current blocker instead of a broad contract gap. `.env.qa.example` and `scripts/verify-qa-env.mjs` already document and enforce `QA_EXTENSION_API_BASE_URL` and `QA_EXTENSION_API_TOKEN` when extension smoke is configured. In the current QA run, raw `corepack pnpm run qa:env` failed only because the shell had not sourced `.env.qa`; after sourcing `.env.qa`, `qa:env` failed only on missing `QA_EXTENSION_API_TOKEN`, and sourced `qa:target-preflight` passed DNS, HTTPS, app shell, and Clerk before stopping on that same token gap. Keep this request open until the token and seeded allow/deny fixtures exist.
