...
- Current status: Supabase MCP access improved in the 2026-05-30/2026-05-31 specialist runs, where read-only SQL, policy catalog queries, security/performance advisors, function ACL checks, grants, safe aggregate counts, edge-function inventory/source, logs, and storage metadata were reported callable for project `vaoqvtxqvbptyxddpoju`. The 2026-05-31 lead run had only partial connector coverage: project metadata plus edge-function inventory/source were callable, but read-only SQL, catalog queries, advisors, storage, and logs were not exposed as callable tools. Keep this request open until SQL, advisors, logs, and catalog inspection are dependable across consultant sessions.
...
- Current status: This gap remains open and changed materially on 2026-05-31. Exporting `.env.qa` with `set -a; source .env.qa; set +a` still restores reachability, and the latest consultant runs now recovered fresh screenshot artifacts for admin, client admin, client finance, and Bum desktop routes plus public mobile routes. Authenticated browser evidence is still not fully stable, though: the current portal interaction audit still stops early because its hash-link assertion treats the shared `#main-content` skip link as broken even when the target exists, and one Chromium client-finance flow hit a transient `ERR_ADDRESS_UNREACHABLE` while returning to `/client/dashboard`. The repo also still has no `@axe-core/playwright` or `axe-core`, no narrated keyboard or screen-reader notes, and no fresh mobile authenticated route captures from this run.
...
- Current status: On 2026-05-31, `pnpm exec playwright test tests/e2e/authenticated-role-smoke.spec.ts --project=chromium` failed 4 of 5 checks. Client Finance and Client Member timed out inside `clerk.signIn()` while Clerk testing reported repeated failures to `https://clerk.trustedbums.com/v1/client/sign_ins`, and Admin plus Client Admin timed out during post-auth route settlement. The current session has no Clerk dashboard evidence or auth logs to distinguish account issues, upstream auth instability, or test-helper defects.
...
- Current status: This gap narrowed on 2026-05-31. Live Supabase inventory now does include `submit-contact`, so the specific contact-intake deployment drift noted on 2026-05-30 has been resolved. The remaining drift question is still material: live inventory still shows `admin-user-tools`, still does not expose a matching live `client-team` deployment path, and the current session still lacks CI/CD history, release notes, extension-release provenance, and secrets inventory by variable name to explain when or why those differences happened.
...
- Current status: This gap also narrowed on 2026-05-31. Live `submit-contact` now verifies Turnstile, checks origin, rate-limits by recent submissions, and writes `contact_submissions` through a service-role path before notifying operations. The remaining concern is that live `send-website-email` is still directly public and mail-sending, so the consultant session still needs Turnstile/dashboard evidence, edge or CDN rate-limit/WAF settings, and sender-reputation operations visibility to prove the public mail path cannot be abused.
...
- Current status: This gap narrowed again on 2026-05-31. Supabase performance advisors, read-only SQL, and recent `performance-beacon` edge-function logs are callable, and live SQL now shows `2,355` `performance_metric_events` rows across `42` routes from one observed origin in the last 7 days. The remaining blocker is diagnostic depth rather than total telemetry absence: there is still no CI history, flaky-test history, deploy logs, browser telemetry dashboard, Lighthouse artifact set, bundle-analyzer output, authenticated route network traces, query-plan output, `pg_stat_statements`, or frontend error-monitoring context.
...
- Current status: The telemetry shortage is no longer the main issue. Live Supabase SQL on 2026-05-31 now shows `2,355` `performance_metric_events` rows across `42` routes in the last 7 days and recent `performance-beacon` logs show repeated `202` writes. The QA env contract is also recoverable in this repo with `set -a; source .env.qa; set +a`. What is still missing is access proof for the admin monitoring surface itself: this performance session did not complete a live admin walkthrough of `/admin/performance` or a non-admin deny check, even though a fresh client-admin Playwright audit did re-confirm authenticated route reachability elsewhere in the portal.
...
- Current status: This gap narrowed but is still open. The 2026-05-31 consultant runs regained live browser evidence: after sourcing `.env.qa`, `pnpm run qa:env` passed and Playwright produced fresh authenticated desktop screenshot artifacts for admin, client admin, client finance, and Bum routes plus fresh public mobile UI captures. What is still missing is the product and design source of truth itself: no brand guidance, component-library references, approved screenshot baselines, analytics, session recordings, support evidence, sales-objection data, or narrated workflow walkthroughs were available in the session, and authenticated mobile portal screenshots are still missing, so UX and UI recommendations remain implementation-ready but not design-approved.
