...
- When a consultant run needs a local preview or route check from the Codex runner, use port `8080` only. When external DNS or hosted reachability context is needed, use `https://trustedbums.com` unless Ryan explicitly specifies another host.
- For GitHub-hosted `QA`, `E2E Smoke`, `Visual UI Audit`, and `Deep QA Hotfix Audit`, keep `QA_BASE_URL` on `https://trustedbums.com` unless Ryan explicitly asks to validate another deployed host. `https://rcdl.tplinkdns.com` is retired and is no longer a fallback DNS, TLS, release-QA, or visual-QA target.
