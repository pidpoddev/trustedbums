...
- When a consultant run needs a local preview or route check from the Codex runner, use port `8080` only. The retired `https://rcdl.tplinkdns.com` target is no longer a required trust, release, QA, or visual-review surface unless Ryan explicitly asks to validate it again.
