...
- When a consultant run needs a local preview or route check from the Codex runner, use port `8080` only. When the run needs external DNS context, use `https://rcdl.tplinkdns.com` as the named external DNS target and report it separately from primary-host trust, release, QA, or visual-review proof.
