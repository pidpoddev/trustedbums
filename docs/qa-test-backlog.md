...
Local preflight quality is stable but not release-complete. Full local QA passes lint, 67 tests across 21 files, and production build. Hosted authenticated role smoke, staging smoke, portal interaction audit, and non-destructive Deep QA pass against `trustedbums.com`. Sourced `qa:env` now intentionally fails until `QA_EXTENSION_API_TOKEN` is supplied, because `.env.qa` already includes `QA_EXTENSION_API_BASE_URL`; anonymous extension API smoke is not enough for the cross-company boundary. Mutating Deep QA remains skipped until cleanup credentials are present.
