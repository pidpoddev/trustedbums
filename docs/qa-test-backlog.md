...
Latest local release evidence improved on 2026-06-08: hosted `authenticated-role-smoke.spec.ts` now passes all five roles against `https://trustedbums.com` after the live Supabase RLS helper grant restoration. The earlier non-admin Client Terms gate failure was caused by `permission denied for function current_company_id` and `permission denied for function is_admin`, and is fixed by live migration `20260608002426 restore_rls_helper_execute_grants`. Follow-up hosted smoke also verifies the older 2026-06-04 GitHub `E2E Smoke` failures are fixed in the current deployment: staging smoke passes current signup validation copy, Client Finance opens payments without duplicate-heading failure, and portal interaction audit routes Client Finance global search to `/client/payments`. Hosted non-destructive Deep QA now reports no hotfix-level issues and all listed Admin, Client, Client Finance, Client Member, and Bum routes passed. GitHub-hosted workflow evidence still needs a fresh review/rerun before treating `main` as release-safe, but the old E2E and route-audit failures should no longer be carried as active product/test defects.

Local preflight quality is stable but not release-complete. Full local QA passes lint, 70 tests across 22 files, and production build. Hosted authenticated role smoke, staging smoke, portal interaction audit, and non-destructive Deep QA pass against `trustedbums.com`. Sourced `qa:target-preflight` now classifies DNS, HTTPS, app shell, Clerk, and extension API readiness; it passes every check except the expected missing `QA_EXTENSION_API_TOKEN`. Sourced `qa:env` also intentionally fails until `QA_EXTENSION_API_TOKEN` is supplied, because `.env.qa` already includes `QA_EXTENSION_API_BASE_URL`; anonymous extension API smoke is not enough for the cross-company boundary. Mutating Deep QA remains skipped until cleanup credentials are present.
...
- Verified this run:
  `pnpm run qa:target-preflight` was added and wired into `E2E Smoke` plus `Deep QA Hotfix Audit`; sourced local execution passed DNS, HTTPS, app shell, and Clerk checks, then failed with a classified missing `QA_EXTENSION_API_TOKEN` extension API readiness issue.
...
- Failed or limited this run:
  Mutating Deep QA skipped because `QA_DEEP_MUTATION=1`, `QA_SUPABASE_URL`, and `QA_SUPABASE_SERVICE_ROLE_KEY` are not configured locally.
