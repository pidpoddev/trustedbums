...
Latest local release evidence improved on 2026-06-08: hosted `authenticated-role-smoke.spec.ts` now passes all five roles against `https://trustedbums.com` after the live Supabase RLS helper grant restoration. The earlier non-admin Client Terms gate failure was caused by `permission denied for function current_company_id` and `permission denied for function is_admin`, and is fixed by live migration `20260608002426 restore_rls_helper_execute_grants`. Follow-up hosted smoke also verifies the older 2026-06-04 GitHub `E2E Smoke` failures are fixed in the current deployment: staging smoke passes current signup validation copy, Client Finance opens payments without duplicate-heading failure, and portal interaction audit routes Client Finance global search to `/client/payments`. GitHub-hosted workflow evidence still needs a fresh review/rerun before treating `main` as release-safe, but the old failure should no longer be carried as an active product/test defect.

Local preflight quality is stable but not release-complete. Full local QA passes lint, 67 tests across 21 files, and production build. Hosted authenticated role smoke, staging smoke, and portal interaction audit pass against `trustedbums.com`. Sourced `qa:env` now intentionally fails until `QA_EXTENSION_API_TOKEN` is supplied, because `.env.qa` already includes `QA_EXTENSION_API_BASE_URL`; anonymous extension API smoke is not enough for the cross-company boundary.
...
### P0 - Rerun deep QA and preserve complete route evidence after the terms-gate fix
- Evidence: Downloaded GitHub artifact from run `26933527284` contains `deep-qa-hotfix-report` files. Both original and retry reports flag `P1 - Client: Terms` with evidence `Unable to accept current terms during E2E sign-in` at `https://trustedbums.com/client/terms`. Follow-up live logs and SQL on 2026-06-08 traced the non-admin terms gate to missing execute grants on RLS helpers; migration `20260608002426` restored the grants, and hosted authenticated role smoke now passes all five roles. The original deep run also flags Client Member routes after timeout, so a fresh deep rerun is still needed.
...
- Recommendation: Rerun deep QA after the grant fix, then bound deep QA per route/account and attach partial reports before timeout so route-specific failures survive even when the global suite hits its cap.
- Acceptance criteria: Client Admin terms acceptance remains green; the Client Member `/client/requests` and `/client/opportunities/new` failures are reproduced or reclassified as timeout fallout; deep QA emits a complete route-result artifact with per-route timeout classification; rerun fails only on explicit P0/P1 findings.
