...
- Evidence: `docs/business-access-rules.md` defines profile bootstrap, extension captures/represented contacts, and performance telemetry as authorization-sensitive. Product Ops flags a concrete represented-contact scoping defect: `portal-contacts` loads all accepted opportunities and accepts existing `opportunity_registration_id` values for Bum contact linking after existence checks. Data/Analytics flags `admin_dashboard_summary()` as a privileged aggregate helper with broad execute exposure. Current tests still do not prove `/bum/contacts/:id`, `performance_metric_events`, `admin_dashboard_summary()`, or profile authorization-field mutation denials.
