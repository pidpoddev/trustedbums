...
const routeInventory: WorkflowRoute[] = [
  ...
  { role: "CLIENT_ADMIN", path: "/client/requests", heading: "Inbound Requests", area: "Client", workflow: "Inbound requests" },
  { role: "CLIENT_ADMIN", path: "/client/team", heading: "Team Management", area: "Client", workflow: "Team management" },
  { role: "CLIENT_FINANCE", path: "/client/payments", heading: "Customer Payments", area: "Client Finance", workflow: "Payments" },
  ...
  { role: "BUM", path: "/bum/claims", heading: "Intro Requests", area: "Bum", workflow: "Intro requests" },
...
async function createClientTarget(page: Page, runId: string, records: QaCreatedRecord[]) {
  ...
  await page.getByLabel("Key contact").fill("QA Contact");
  await page.getByLabel("Key contact email").fill("qa-contact@example.com");
...
async function createClientOpportunity(page: Page, runId: string, records: QaCreatedRecord[]) {
  ...
  await page.getByLabel("Customer account name").fill(name);
  await page.getByLabel("Business unit / department").fill("QA");
  await page.getByLabel("Your internal contact").fill("QA Client");
  await page.getByLabel("Trusted Bums owner").fill("QA Owner");
  await page.getByLabel("Expected product/service").fill("Deep QA validation");
  await page.getByLabel("Estimated deal value").fill("50000");
