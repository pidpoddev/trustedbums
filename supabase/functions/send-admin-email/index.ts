...
function htmlFromText(value: string, deliveryId?: string) { let escaped = escapeHtml(value); if (deliveryId) { escaped = escaped.replace(/https?:\/\/[^\s<]+/g, (url) => { const trackingUrl = `${emailTrackingBaseUrl}/click?d=${encodeURIComponent(deliveryId)}&u=${encodeURIComponent(url)}`; return `<a href="${trackingUrl}">${url}</a>` }); } const pixel = deliveryId ? `<img src="${emailTrackingBaseUrl}/open?d=${encodeURIComponent(deliveryId)}" width="1" height="1" alt="" style="display:none" />` : ""; return `${escaped.replace(/\n/g, "<br>")}${pixel}` }
